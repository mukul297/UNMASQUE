...
def generateCombos(val):
	print("inside -- where_clause.generateCombos")
	res = [[0]]
...
def get_all_combo_lists(max_list_len):
	print("inside -- where_clause.get_all_combo_lists")
	result = {0:[], 1:[]}
...
def get_join_graph():
	print("inside -- where_clause.get_join_graph")
	get_init_data()
	max_list_len = 2
	join_graph = []
	key_attributes = []
...
def update_other_data(tabname, attrib, attrib_type, val, result, other_info_list):
	print("inside -- where_clause.update_other_data")
	reveal_globals.local_other_info_dict = {}
...
def is_int(s):
	print("inside -- where_clause.is_int")
	try:
		int(s)
		return True
	except ValueError:
...
def get_init_data():
	print("inside -- where_clause.get_init_data")
	...
	for tabname in reveal_globals.global_core_relations:
		...
		cur.close()
		tab_attribs = []
		for row in res: # this loop will give column_name by table wise
...
def checkAttribValueEffect(tabname, attrib, val):
	print("inside -- where_clause.checkAttribValueEffect")
	#updatequery
...
def getIntFilterValue(tabname, filter_attrib, min_val, max_val, operator):
	print("inside -- where_clause.getIntFilterValue")
	counter = 0
...
def getDateFilterValue(tabname, attrib, min_val, max_val, operator):
	print("inside -- where_clause.getDateFilterValue")
	counter = 0
...
def checkStringPredicate(tabname, attrib):
	print("inside -- where_clause.checkStringPredicate")
	#updatequery
	val = ''
...
def getStrFilterValue(tabname, attrib, representative, max_length):
	print("inside -- where_clause.getStrFilterValue")
	index = 0
	output = ""
...
def get_filter_predicates():
	print("inside -- where_clause.get_filter_predicates")
	reveal_globals.local_other_info_dict = {}
